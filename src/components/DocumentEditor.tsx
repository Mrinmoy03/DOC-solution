...
export const DocumentEditor = ({ file, onClose, onSave, createNew }: DocumentEditorProps) => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-sm sm:prose lg:prose-lg xl:prose-2xl mx-auto focus:outline-none min-h-[1056px]',
      },
    },
    onUpdate: ({ editor }) => {
  ...
  const handlePageSetupApply = (settings: any) => {
    const dpi = 96;
    let width = 816;
    let height = 1056;
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex overflow-hidden relative bg-[#F9FBFD]">
            {ruler.showRuler && (
              <div className="flex-shrink-0 sticky left-0 z-10 h-full">
                <VerticalRuler />
              </div>
            )}
