...
import {
    ...
    Undo, Redo, Printer, Type, Highlighter, CheckSquare,
    ChevronDown, Strikethrough, Superscript, Subscript, Check, Omega, Activity
} from 'lucide-react';
import { InsertChartModal } from '../../components/modals/InsertChartModal';
...
import { InsertSymbolModal } from '../../components/modals/InsertSymbolModal';
...
export const EditorToolbar = () => {
    ...
    const [showUnderlineColorPicker, setShowUnderlineColorPicker] = useState(false);
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (!target.closest('.highlight-color-picker-container')) {
                setShowHighlight(false);
            }
        };

        if (showFontMenu || showFontSizeMenu || showTextColor || showHighlight) {
            document.addEventListener('mousedown', handleClickOutside);
            return () => document.removeEventListener('mousedown', handleClickOutside);
        }
    }, [showFontMenu, showFontSizeMenu, showTextColor, showHighlight]);

    // Close font/font size/color menus when other menus open
    useEffect(() => {
        if (showZoomMenu || showStyleMenu) {
            setShowFontMenu(false);
            setShowFontSizeMenu(false);
            setShowTextColor(false);
            setShowHighlight(false);
        }
    }, [showZoomMenu, showStyleMenu]);
    ...
    const toggleUnderline = () => {
        ...
        if (!isUnderline) {
            setShowUnderlineColorPicker(true);
        } else {
            setShowUnderlineColorPicker(false);
        }
    };
    const setUnderlineColor = (color: string) => {
        editor?.chain().focus().setUnderlineColor(color).run();
    };
    const setUnderlineStyle = (style: string) => {
        editor?.chain().focus().setUnderlineStyle(style).run();
    };
    ...
    const handleInsertLink = (url: string, text?: string) => {
        ...
        if (text) {
            if (editor.state.selection.empty) {
                ...
                const activeDoc = editor.state.doc;
                const selectedText = activeDoc ? activeDoc.textBetween(from, to, ' ') : '';
    ...
    return (
        <div className="relative z-20 bg-[#edf2fa] px-4 py-1.5 flex items-center gap-1.5 border-b border-gray-300 flex-wrap">
            ...
            <div className="flex items-center gap-0.5 border-r border-gray-300 pr-2 mr-1">
                ...
                {/* Highlight Color */}
                <div className="relative highlight-color-picker-container">
                    <ToolbarButton onClick={() => {
                    ...
                    {showHighlight && (
                        <ColorPicker
                            ...
                            initialColor={editor?.getAttributes('highlight').color}
                        />
                    )}
                </div>
            </div>
            ...
            <div className="flex items-center gap-0.5 border-r border-gray-300 pr-2 mr-1">
                ...
                <ToolbarButton onClick={() => setShowChartModal(true)} title="Insert chart">
                    <Activity size={16} />
                </ToolbarButton>
            </div>
            ...
            <div className="flex items-center gap-0.5 border-r border-gray-300 pr-2 mr-1">
                ...
                <ToolbarButton onClick={() => setShowSymbolModal(true)} title="Insert Symbol">
                    <Omega size={16} />
                </ToolbarButton>
            </div>
            ...
            {showUnderlineColorPicker && (
                <DraggableColorPicker
                    onClose={() => setShowUnderlineColorPicker(false)}
                    onColorSelect={setUnderlineColor}
                    onStyleSelect={setUnderlineStyle}
                    title="Underline Options"
                    initialColor={editor?.getAttributes('underline').color}
                    initialStyle={editor?.getAttributes('underline').style}
                />
            )}
        </div>
    );
...
const ColorPicker = ({ onChange, onClose, initialColor }: { onChange: (color: string) => void, onClose: () => void, initialColor?: string }) => {
    const [hex, setHex] = useState(initialColor || '#000000');
    const colors = [
    ...
    return (
        <div className="absolute top-full left-0 mt-1 bg-white border border-gray-300 rounded shadow-lg z-50 p-2 w-56">
            ...
            <div className="flex items-center gap-2 mb-2 p-1" onMouseDown={(e) => e.stopPropagation()}>
                <div className="relative w-6 h-6 flex-shrink-0 border border-gray-300 rounded overflow-hidden">
                    <input
                        type="color"
                        ...
                        className="absolute inset-0 opacity-0 w-full h-full cursor-pointer z-10 p-0 m-0 border-0"
                    />
                    <div
                        className="w-full h-full"
                        style={{ backgroundColor: hex.length === 7 ? hex : '#000000' }}
                    />
                </div>
                <input
                    type="text"
                    ...
                    className="flex-1 text-xs border border-gray-300 rounded px-1 py-1 focus:outline-none focus:border-blue-400 uppercase font-mono"
                    placeholder="#000000"
            ...
            <button
                onClick={onClose}
                className="w-full text-xs text-gray-600 hover:text-gray-800 text-center pt-1 border-t border-gray-100"
            >
                Close
            </button>
        </div>
